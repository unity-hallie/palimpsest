...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (nightness > 0.01 && uv.y < tideH) {
        ...
        const float LAT    = 0.7767;   // 44.5N (Maine) in radians
        const float sinLAT = 0.7009;
        const float cosLAT = 0.7133;
        float lst = fract(iTime / CYCLE_TIME) * 6.28318; // LST 0-2
        ...
        // All stars rotate correctly because they live in celestial coords.
        {
            ...
            if (abs(sinAz0) < 0.999) {
                // Force southern sky: az= at center, az=/2(E) left, az=3/2(W) right
                float az0  = 3.14159265 - asin(sinAz0);
                // Alt/az -> Dec, H (full atan2 so H  [-, ])
                ...
                // Cellular hash in RA/Dec space -- 33 neighborhood
        ...
        for (int i = 0; i < 35; i++) {
            float ra_rad  = cat[i].x * 0.26180;   // hours -> radians (*/12)
            float dec_rad = cat[i].y * 0.01745;   // degrees -> radians (*/180)
            float mag     = cat[i].z;
            float ctype   = cat[i].w;
            ...
            float az     = atan(sinAz, cosAz);  // 0=N, =S
