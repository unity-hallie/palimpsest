...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Decouple visual brightness from refractory state.
    // The refractory gate lives at 0.44 in the compute shader -- cells below that
    // are ready to fire again. But the slow decay floor keeps emission near 0.44-0.50
    // for minutes, which would make the ocean glow long after a scroll.
    // Fix: dim anything in the slow floor. Full flash (>0.55) is unaffected.
    // Refractory gate and slow decay rate are unchanged -- only the visual mapping shifts.
    float dimFactor   = smoothstep(0.20, 0.55, emission);
    float visEmission = emission * mix(0.08, 1.0, dimFactor);
    float visBloom    = bloom    * mix(0.08, 1.0, dimFactor);

    float core = pow(visEmission, 2.8);
    float halo = pow(visBloom, 2.0);
